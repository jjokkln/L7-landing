...
export function FocusRail({
    ...
    return (
        <div
            className={cn(
                "group relative flex h-[600px] w-full flex-col overflow-hidden bg-white text-black outline-none select-none overflow-x-hidden",
                className
            )}
        ...
        >
            {/* Background Ambience 
       Changed to white background paradigm so removing dark mode blurs/gradients 
       and replacing with lighter ones if needed, or keeping them subtle.
      */}
            <div className="absolute inset-0 z-0 pointer-events-none">
                <AnimatePresence mode="popLayout">
                    <motion.div
                        ...
                        initial={{ opacity: 0 }}
                        animate={{ opacity: 0.1 }} // Reduced opacity for light theme
                        exit={{ opacity: 0 }}
            ...
            <div className="relative z-10 flex flex-1 flex-col justify-center px-4 md:px-8">
                ...
                >
                    {visibleIndices.map((offset) => {
                        ...
                        const opacity = isCenter ? 1 : Math.max(0.3, 1 - dist * 0.5); // Adjusted min opacity for light theme
                ...
                <div className="mx-auto mt-12 flex w-full max-w-4xl flex-col items-center justify-between gap-6 md:flex-row pointer-events-auto">
                    ...
                    <div className="flex items-center gap-4">
                        <div className="flex items-center gap-1 rounded-full bg-gray-100 p-1 ring-1 ring-black/5 backdrop-blur-md">
                            <button
                                onClick={handlePrev}
                                className="rounded-full p-3 text-gray-500 transition hover:bg-white hover:text-black hover:shadow-sm active:scale-95"
                                aria-label="Previous"
                            >
                        ...
                        {activeItem.href && (
                            <Link
                                ...
                                className="group flex items-center gap-2 rounded-full bg-black px-5 py-3 text-sm font-semibold text-white transition-transform hover:scale-105 active:scale-95"
                            >
                                Explore
